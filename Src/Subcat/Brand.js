...
const Brand = ({ route, navigation }) => {
    ...
    useEffect(() => {
        fetch('http://192.168.158.193/demo/ecommerce/api_brand.php?subcat_id=' + root.sub_id)
            .then(res => res.json())
            .then(
                (result) => {
                    console.log(result)
                    ...
                    <View style = {{ margin: 8, flexWrap: "wrap" }} >
                        <View style = {{ width: 190, height: 190, elevation: 5, backgroundColor: "black", borderRadius: 15, justifyContent: 'center', alignItems: 'center' }} >
                            <TouchableOpacity onPress = {() => navigation.navigate("Product")} >
                                <Image source = {{ uri: "http://192.168.158.193/demo/ecommerce/Brand%20Image/" + item.brand_img }} style = {{ width: 190, height: 164, borderRadius: 15 }}/>  <Text style = { styles.cattxt } > { item.brand } </Text> 
                            </TouchableOpacity> 
                        </View> 
                    </View>
                    </>
